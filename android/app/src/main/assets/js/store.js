...
function seedState(){
  ...
  const mk=(name,icon,freq,perDay,streakNow,longestAt,created,missRate)=>{
    const hist=[]; let counts={};
    for(let i=1;i<=90;i++){
      const d=addDays(today,-i);
      let hit=rnd()>missRate;
      if(hit){ hist.push(d); if(perDay>1) counts[d]=perDay; }
    }
    ...
    [streakNow+1,streakNow+2,streakNow+3+longestAt].forEach(i=>{
      const d=addDays(today,-i), k=hist.indexOf(d); if(k>-1) hist.splice(k,1); delete counts[d];
    });
    return { id:"g"+Math.random().toString(36).slice(2,8), name, icon, freq, perDay, createdAt:created, history:hist, counts, earlyFlags:{} };
  };
  ...
  return {
    ...
    energy:620, flags:{earlyBird:true},
